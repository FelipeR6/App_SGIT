...
router.post('/', async (req, res) => {
  try {
    ...
    const result = await db.query('INSERT INTO usuario SET ?', [nuevoUsuario]);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const result = await db.query('UPDATE usuario SET ? WHERE Id_Usuario = ?', [usuarioActualizado, id]);
...
router.post('/login', async (req, res) => {
  try {
    ...
    const usuarioEncontrado = await db.query('SELECT * FROM usuario WHERE Usuario = ? AND Contraseña = ? AND Rol = ?', [usuario, password, rol]);
    ...
    // También actualiza las referencias posteriores a 'usuario' por 'usuarioEncontrado'
    res.json({
      success: true,
...
router.post('/register', async (req, res) => {
  try {
    ...
    const usuarioExistente = await db.query('SELECT * FROM usuario WHERE Usuario = ?', [nuevoUsuario.Usuario]);
